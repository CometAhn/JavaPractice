...
public class _220708프로그램설계2 {
  ...
  public static void main(String[] args) {
    ...
    // 학점 = C
    //
    // 문제3. 입력 받은 양의 정수가 3의 배수, 5의 배수 혹은 8의 배수인지를 알려주는 프로그램을 설계하고 작성하라. 양의 정수가 3의 배수이면 '3의 배수이다'를
    // 출력하고 5의 배수이면 '5의 배수이다'를 출력하며 8의 배수이면 '8의 배수이다'를 출력하고 그 외의 경우이면 '어느 배수도 아니다'를 출력하라.
    //
    // 문제4. 할인점에서 고객에게 청구 금액을 계산하는 프로그램을 설계하고 작성하라. 고객이 30만원 이상 구매하면 3만원을 할인해 주어야 하고 30만원 미만 10만원
    // 이상을 구매하면 5,000원을 할인해 주어야 하며 10만원 미만을 구매하면 할인이 없어야 한다. 키보드로부터 구매 금액을 입력 받아야 한다. 구매 금액과 청구 금액을
    // 출력해야 한다.
    //
    ...
    // 연간 근로소득은 입력받아야 한다.
    //
    // 문제6. 할인점에서 가격이 100,000원인 한 제품을 판매한다. 그 제품을 10개 이상 사는 고객에게 10%를 할인해 준다. 고객이 구입한 그 제품의 수량을 입력하면
    // 총 가격을 계산하는 프로그램을 설계하고 작성하라.
    //
    // 문제7. 영화관에서 상영중인 영화에 대해 고객 나이가 19세 이상이면 "영화를 볼 수 있습니다"를 출력하고 나이가 19세 미만이고 15세 이상이면 "성인이 동반한
